...
# Remove project root sys.path mutation to avoid side effects
...
class Veo3ClientManager:
    ...
    def get_genai_client(self) -> genai.Client:
        ...
        if self._genai_client is None:
            ...
            if self.config.DEBUG:
                ...
                logger.info("🎬 Veo3 Model: %s", self.config.VEO3_MODEL)
                logger.info("🧠 Gemini Model: %s", self.config.GEMINI_MODEL)
    ...
    def get_video_generation_config(
        ...
        *,
        ...
        """
        # Dynamically build kwargs so we don't send unsupported None values
        kwargs: Dict[str, Any] = {
            "aspect_ratio": aspect_ratio or self.config.DEFAULT_ASPECT_RATIO,
            "number_of_videos": 1,
        }

        # TODO: Uncomment these when the model supports them (Google API is not updated yet)
        # Optional params – only include when supported/non-None
        # if duration_seconds is not None:
        #     kwargs["duration_seconds"] = duration_seconds
        # if resolution is not None:
        #     kwargs["resolution"] = resolution

        # Person generation enum fix: default to "dont_allow" instead of invalid "allow_none"
        # if person_generation is not None:
        #     kwargs["person_generation"] = person_generation
        # else:
        #     kwargs["person_generation"] = (
        #         "allow_adult" if self.config.ENABLE_ADULT_GENERATION else "dont_allow"
        #     )
